...
########
# Compute auxiliary edge fields
########

E_edge_0 = N.zeros(Nxi + 2 * NG)
E_edge_1 = N.zeros(Neta + 2 * NG)
B_edge_0 = N.zeros(Nxi + 2 * NG)
B_edge_1 = N.zeros(Neta + 2 * NG)

def compute_E_edge(p0, p1, label, top):
    ...
    if (top == 0):
        ...
        for j in range(NG, Neta + NG):
            ...
            # Average to get E_eta at eta, not eta_yee
            E_xi_0  = field1[p0, Nxi + NG - 1, j]    
            E_eta_0 = 0.5 * (field2[p0, Nxi + NG - 1, j] + field2[p0, Nxi + NG - 1, j - 1])
            ...
            # E_eta at Nxi + NG - 1/2, j + 1/2
            denom = 4.0 - J_0to1[0, 1] * J_1to0[1, 0]      
            E_edge_0[j] = (2.0 * E_eta_0 + 2.0 * J_1to0[1, 1] * E_eta_1 + J_1to0[1, 0] * (E_xi_1 + J_0to1[0, 0] * E_xi_0)) / denom
            # E_xi_b at NG, j 
...
def compute_B_edge(p0, p1, label):
...
def communicate_E_patch(patch0, patch1):
    ...
    if (top == 0):
        return
            ...
            compute_E_edge(patch0, patch1, 'vec', top)
            ...
            compute_E_edge(patch0, patch1, 'form', top)
            ...
            E1u1 = interp(E2u[patch1, :, 1], xi_yee, xi1)
            E2u1 = interp(E_edge_1, xi, xi1)
            ...
            E1d1 = interp(E2d[patch1, :, 1], xi_yee, xi1)
            E2d1 = interp(E_edge_1, xi, xi1)
            ...
            E1d1 = interp(E1d[patch1, :, 1], xi, xi1)
            E2d1 = interp(E_edge_1, xi, xi1)
...
def communicate_B_patch(patch0, patch1):
    ...
    if (top == 0):
        return
            ...
            compute_B_edge(patch0, patch1, 'vec', top)
            ...
            B1u0 = interp(B_edge_0, eta_yee, eta0)
            ...
            B1u1 = interp(B1u[patch1, 1, :], eta, eta1)
            B2u1 = interp(B_edge_1, eta, eta1)
            ...
            compute_B_edge(patch0, patch1, 'form', top)
            ...
            B1d0 = interp(B1d[patch0, :, -2], xi, xi0)
            B2d0 = interp(B_edge_0, xi_yee, xi0)
