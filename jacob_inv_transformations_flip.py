...
def jacob_inv_A_to_sph(xi, eta):
    ...
    return N.array([[0.0, - delta**2 / (C * D * D)], [1.0, X * Y / D**2]])
...
def jacob_inv_sph_to_A(theta, phi):
    ...
    D = N.sqrt(1.0 + Y * Y)
    delta = N.sqrt(1.0 + X * X + Y * Y)
    return N.array([[C * X * Y / delta**2, 1.0], [- C * D * D / delta**2, 0.0]])
...
def jacob_inv_C_to_sph(xi, eta, vxi, veta):
    X = N.tan(xi)
    Y = N.tan(eta)
    C = N.sqrt(1.0 + X * X)
    D = N.sqrt(1.0 + Y * Y)
    delta = N.sqrt(1.0 + X * X + Y * Y)
    return N.array([[delta**2 / (C * C * D), 0.0], [X * Y / C**2, 1.0]])

def jacob_inv_sph_to_C(theta, phi, vth, vph):
    X = 1.0 / N.tan(theta) / N.cos(phi)
    Y = N.tan(phi)
    C = N.sqrt(1.0 + X * X)
    D = N.sqrt(1.0 + Y * Y)
    delta = N.sqrt(1.0 + X * X + Y * Y)
    vxi = (delta**2 / (C * C * D)) * vth + (X * Y / C**2) * vph
    veta = vph
    return N.array([[C * C * D / delta**2, 0.0], [- D * X * Y / delta**2, 1.0]])

########################

def jacob_inv_D_to_sph(xi, eta, vxi, veta):
    X = N.tan(xi)
    Y = N.tan(eta)
    C = N.sqrt(1.0 + X * X)
    D = N.sqrt(1.0 + Y * Y)
...
def jacob_inv_sph_to_D(theta, phi, vth, vph):
