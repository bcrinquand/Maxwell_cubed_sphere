...
r_int = N.array(h5f['r_int'])
r_half = N.array(h5f['r_half'])
...
# Initialize fields
Dru = N.zeros((n_patches, Nr_half, Nxi_int, Neta_int))
D1u = N.zeros((n_patches, Nr_int, Nxi_half, Neta_int))
D2u = N.zeros((n_patches, Nr_int, Nxi_int, Neta_half))

Dr_c = N.zeros((n_patches, Nr, Nxi, Neta))
D1_c = N.zeros((n_patches, Nr, Nxi, Neta))
D2_c = N.zeros((n_patches, Nr, Nxi, Neta))
...
DrS = N.zeros((Nr, ONt, ONp))
DtS = N.zeros((Nr, ONt, ONp))
DpS = N.zeros((Nr, ONt, ONp))
...
def compute_D_nodes(p):
    ...
    Dr_c[p, :, :, :] = 0.25 * (Dru[p, 1:-1, :-1, :-1] + N.roll(Dru, -1, axis = 2)[p, 1:-1, :-1, :-1] + N.roll(Dru, -1, axis = 3)[p, 1:-1, :-1, :-1] \
                            +  N.roll(N.roll(Dru, -1, axis = 2), -1, axis = 3)[p, 1:-1, :-1, :-1])    
    
    D1_c[p, :, :, :] = 0.25 * (D1u[p, :-1, 1:-1, :-1] + N.roll(D1u, -1, axis = 1)[p, :-1, 1:-1, :-1] + N.roll(D1u, -1, axis = 3)[p, :-1, 1:-1, :-1] \
                            +  N.roll(N.roll(D1u, -1, axis = 1), -1, axis = 3)[p, :-1, 1:-1, :-1])

    D2_c[p, :, :, :] = 0.25 * (D2u[p, :-1, :-1, 1:-1] + N.roll(D2u, -1, axis = 1)[p, :-1, :-1, 1:-1] + N.roll(D2u, -1, axis = 2)[p, :-1, :-1, 1:-1] \
                            +  N.roll(N.roll(D2u, -1, axis = 1), -1, axis = 2)[p, :-1, :-1, 1:-1])
...
for h in tqdm(range(Nr)):
    ...
    for patch in range(6):
        ...
        PB1D = PB1D + (Dr_c[patch, h, :, :] * r_half[1 + h]).flatten().tolist()
...
R, THETA, PHI = N.meshgrid(r_half[1:-1], Ot, Op)
...
h5f.create_dataset('rpos',  data=r_half[1:-1])
