...
# Initialize fields
Bru = N.zeros((n_patches, Nr_int, Nxi_half, Neta_half))
B1u = N.zeros((n_patches, Nr_half, Nxi_int, Neta_half))
B2u = N.zeros((n_patches, Nr_half, Nxi_half, Neta_int))

Br_c = N.zeros((n_patches, Nr, Nxi, Neta))
B1_c = N.zeros((n_patches, Nr, Nxi, Neta))
B2_c = N.zeros((n_patches, Nr, Nxi, Neta))
...
BrS = N.zeros((Nr, ONt, ONp))
BtS = N.zeros((Nr, ONt, ONp))
BpS = N.zeros((Nr, ONt, ONp))
...
def compute_B_nodes(p):
    ...
    Br_c[p, :, :, :] = 0.5 * (Bru[p, :-1, 1:-1, 1:-1] + N.roll(Bru, -1, axis = 1)[p, :-1, 1:-1, 1:-1])    
    
    B1_c[p, :, :, :] = 0.5 * (B1u[p, 1:-1, :-1, 1:-1]  + N.roll(B1u, -1, axis = 2)[p, 1:-1, :-1, 1:-1])
    B2_c[p, :, :, :] = 0.5 * (B2u[p, 1:-1, 1:-1, :-1]  + N.roll(B2u, -1, axis = 3)[p, 1:-1, 1:-1, :-1])
...
for h in tqdm(range(Nr)):
    ...
    for patch in range(6):
        ...
        PB1D = PB1D + (Br_c[patch, h, :, :] * r_half[1 + h]).flatten().tolist()
