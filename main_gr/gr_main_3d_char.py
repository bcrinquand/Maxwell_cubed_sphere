...
def cov_to_contra_E(p, Erin, E1in, E2in):
    ...
    # Interior
    Eru[p, 1:-1, 1:-1, 1:-1] = hrru_Dr[p, 1:-1, 1:-1, 1:-1] *  Erin[p, 1:-1, 1:-1, 1:-1] \
                      + 0.25 * hr1u_Dr[p, 1:-1, 1:-1, 1:-1] * (E1in[p, 1:, 1:-2, 1:-1] + N.roll(N.roll(E1in, 1, axis = 1), -1, axis = 2)[p, 1:, 1:-2, 1:-1]  \
                                                                                       + N.roll(E1in, 1, axis = 1)[p, 1:, 1:-2, 1:-1] + N.roll(E1in, -1, axis = 2)[p, 1:, 1:-2, 1:-1]) \
                      + 0.25 * hr2u_Dr[p, 1:-1, 1:-1, 1:-1] * (E2in[p, 1:, 1:-1, 1:-2] + N.roll(N.roll(E2in, 1, axis = 1), -1, axis = 3)[p, 1:, 1:-1, 1:-2]  \
                                                                                       + N.roll(E2in, 1, axis = 1)[p, 1:, 1:-1, 1:-2] + N.roll(E2in, -1, axis = 3)[p, 1:, 1:-1, 1:-2]) \

    # Left/right face
    for j in [0, -1]:
        Eru[p, 1:-1, j, 1:-1] = hrru_Dr[p, 1:-1, j, 1:-1] *  Erin[p, 1:-1, j, 1:-1] \
                              + 0.5  * hr1u_Dr[p, 1:-1, j, 1:-1] * (E1in[p, 1:, j, 1:-1] + N.roll(E1in, 1, axis = 1)[p, 1:, j, 1:-1]) \
                              + 0.25 * hr2u_Dr[p, 1:-1, j, 1:-1] * (E2in[p, 1:, j, 1:-2] + N.roll(N.roll(E2in, 1, axis = 1), -1, axis = 3)[p, 1:, j, 1:-2]  \
                                                                                       + N.roll(E2in, 1, axis = 1)[p, 1:, j, 1:-2] + N.roll(E2in, -1, axis = 3)[p, 1:, j, 1:-2]) \
    # Top/bottom face
    for k in [0, -1]:
        Eru[p, 1:-1, 1:-1, k] = hrru_Dr[p, 1:-1, 1:-1, k] * Erin[p, 1:-1, 1:-1, k] \
                                 + 0.25 * hr1u_Dr[p, 1:-1, 1:-1, k] * (E1in[p, 1:, 1:-2, k] + N.roll(N.roll(E1in, 1, axis = 1), -1, axis = 2)[p, 1:, 1:-2, k]  \
                                                                              +   N.roll(E1in, 1, axis = 1)[p, 1:, 1:-2, k] + N.roll(E1in, -1, axis = 2)[p, 1:, 1:-2, k]) \
                                 + 0.5  * hr2u_Dr[p, 1:-1, 1:-1, k] * (E2in[p, 1:, 1:-1, k] + N.roll(E2in, 1, axis = 1)[p, 1:, 1:-1, k])
                                 
    # Back/front face
    for i in [0, -1]:
        Eru[p, i, 1:-1, 1:-1] = hrru_Dr[p, i, 1:-1, 1:-1] * Erin[p, i, 1:-1, 1:-1] \
                              + 0.5 * hr1u_Dr[p, i, 1:-1, 1:-1] * (E1in[p, i, 1:-2, 1:-1] + N.roll(E1in, -1, axis = 2)[p, i, 1:-2, 1:-1]) \
                              + 0.5 * hr2u_Dr[p, i, 1:-1, 1:-1] * (E2in[p, i, 1:-1, 1:-2] + N.roll(E2in, -1, axis = 3)[p, i, 1:-1, 1:-2])
    
    # Edges
    for i in [0, -1]:
        for j in [0, -1]:
            Eru[p, i, j, 1:-1] = hrru_Dr[p, i, j, 1:-1] * Erin[p, i, j, 1:-1] \
                               + hr1u_Dr[p, i, j, 1:-1] * E1in[p, i, j, 1:-1] \
                               + 0.5 * hr2u_Dr[p, i, j, 1:-1] * (E2in[p, i, j, 1:-2] + N.roll(E2in, -1, axis = 3)[p, i, j, 1:-2]) 

    for i in [0, -1]:
        for k in [0, -1]:
            Eru[p, i, 1:-1, k] = hrru_Dr[p, i, 1:-1, k] * Erin[p, i, 1:-1, k] \
                                 + 0.5 * hr1u_Dr[p, i, 1:-1, k] * (E1in[p, i, 1:-2, k] + N.roll(E1in, -1, axis = 2)[p, i, 1:-2, k]) \
                                 + hr2u_Dr[p, i, 1:-1, k] * E2in[p, i, 1:-1, k]

    for j in [0, -1]:
        for k in [0, -1]:
            Eru[p, 1:-1, j, k] = hrru_Dr[p, 1:-1, j, k] *  Erin[p, 1:-1, j, k] \
                              + 0.5 * hr1u_Dr[p, 1:-1, j, k] * (E1in[p, 1:, j, k] + N.roll(E1in, 1, axis = 1)[p, 1:, j, k]) \
                              + 0.5 * hr2u_Dr[p, 1:-1, j, k] * (E2in[p, 1:, j, k] + N.roll(E2in, 1, axis = 1)[p, 1:, j, k])
                               
    # Corners
    for i in [0, -1]:
        for j in [0, -1]:
            for k in [0, -1]:
                Eru[p, i, j, k] = hrru_Dr[p, i, j, k] * Erin[p, i, j, k] \
                                + hr1u_Dr[p, i, j, k] * E1in[p, i, j, k] \
                                + hr2u_Dr[p, i, j, k] * E2in[p, i, j, k] 
    ...
    # Interior
    E1u[p, 1:-1, 1:-1, 1:-1] = h11u_D1[p, 1:-1, 1:-1, 1:-1] * E1in[p, 1:-1, 1:-1, 1:-1] \
                             + 0.25 * h12u_D1[p, 1:-1, 1:-1, 1:-1] * (E2in[p, 1:-1, 1:, 1:-2] + N.roll(N.roll(E2in, 1, axis = 2), -1, axis = 3)[p, 1:-1, 1:, 1:-2] \
                                                                                    +  N.roll(E2in, 1, axis = 2)[p, 1:-1, 1:, 1:-2] + N.roll(E2in, -1, axis = 3)[p, 1:-1, 1:, 1:-2]) \
                             + 0.25 * hr1u_D1[p, 1:-1, 1:-1, 1:-1] * (Erin[p, 1:-2, 1:, 1:-1] + N.roll(N.roll(Erin, -1, axis = 1), 1, axis = 2)[p, 1:-2, 1:, 1:-1] \
                                                                                    +  N.roll(Erin, -1, axis = 1)[p, 1:-2, 1:, 1:-1] + N.roll(Erin, 1, axis = 2)[p, 1:-2, 1:, 1:-1])

    # Left/right face
    for j in [0, -1]:
        E1u[p, 1:-1, j, 1:-1] = h11u_D1[p, 1:-1, j, 1:-1] * E1in[p, 1:-1, j, 1:-1] \
                             + 0.5 * h12u_D1[p, 1:-1, j, 1:-1] * (E2in[p, 1:-1, j, 1:-2] + N.roll(E2in, -1, axis = 3)[p, 1:-1, j, 1:-2]) \
                             + 0.5 * hr1u_D1[p, 1:-1, j, 1:-1] * (Erin[p, 1:-2, j, 1:-1] + N.roll(Erin, -1, axis = 1)[p, 1:-2, j, 1:-1])

    # Top/bottom face
    for k in [0, -1]:
        E1u[p, 1:-1, 1:-1, k] = h11u_D1[p, 1:-1, 1:-1, k] * E1in[p, 1:-1, 1:-1, k] \
                             + 0.5  * h12u_D1[p, 1:-1, 1:-1, k] * (E2in[p, 1:-1, 1:, k] +  N.roll(E2in, 1, axis = 2)[p, 1:-1, 1:, k]) \
                             + 0.25 * hr1u_D1[p, 1:-1, 1:-1, k] * (Erin[p, 1:-2, 1:, k] + N.roll(N.roll(Erin, -1, axis = 1), 1, axis = 2)[p, 1:-2, 1:, k] \
                                                                                    +  N.roll(Erin, -1, axis = 1)[p, 1:-2, 1:, k] + N.roll(Erin, 1, axis = 2)[p, 1:-2, 1:, k])
                                 
    # Back/front face
    for i in [0, -1]:
        E1u[p, i, 1:-1, 1:-1] = h11u_D1[p, i, 1:-1, 1:-1] * E1in[p, i, 1:-1, 1:-1] \
                             + 0.25 * h12u_D1[p, i, 1:-1, 1:-1] * (E2in[p, i, 1:, 1:-2] + N.roll(N.roll(E2in, 1, axis = 2), -1, axis = 3)[p, i, 1:, 1:-2] \
                                                                                    +  N.roll(E2in, 1, axis = 2)[p, i, 1:, 1:-2] + N.roll(E2in, -1, axis = 3)[p, i, 1:, 1:-2]) \
                             + 0.5 * hr1u_D1[p, i, 1:-1, 1:-1] * (Erin[p, i, 1:, 1:-1] + N.roll(Erin, 1, axis = 2)[p, i, 1:, 1:-1])
    
    # Edges
    for i in [0, -1]:
        for j in [0, -1]:
            E1u[p, i, j, 1:-1] = h11u_D1[p, i, j, 1:-1] * E1in[p, i, j, 1:-1] \
                                + 0.5 * h12u_D1[p, i, j, 1:-1] * (E2in[p, i, j, 1:-2] + N.roll(E2in, -1, axis = 3)[p, i, j, 1:-2]) \
                                + hr1u_D1[p, i, j, 1:-1] * Erin[p, i, j, 1:-1]

    for i in [0, -1]:
        for k in [0, -1]:
            E1u[p, i, 1:-1, k] = h11u_D1[p, i, 1:-1, k] * E1in[p, i, 1:-1, k] \
                             + 0.5 * h12u_D1[p, i, 1:-1, k] * (E2in[p, i, 1:, k] + N.roll(E2in, 1, axis = 2)[p, i, 1:, k]) \
                             + 0.5 * hr1u_D1[p, i, 1:-1, k] * (Erin[p, i, 1:, k] + N.roll(Erin, 1, axis = 2)[p, i, 1:, k])

    for j in [0, -1]:
        for k in [0, -1]:
            E1u[p, 1:-1, j, k] = h11u_D1[p, 1:-1, j, k] * E1in[p, 1:-1, j, k] \
                             + h12u_D1[p, 1:-1, j, k] * E2in[p, 1:-1, j, k] \
                             + 0.5 * hr1u_D1[p, 1:-1, j, k] * (Erin[p, 1:-2, j, k] + N.roll(Erin, -1, axis = 1)[p, 1:-2, j, k])
                               
    # Corners
    for i in [0, -1]:
        for j in [0, -1]:
            for k in [0, -1]:
                E1u[p, i, j, k] = h11u_D1[p, i, j, k] * E1in[p, i, j, k] \
                                + h12u_D1[p, i, j, k] * E2in[p, i, j, k] \
                                + hr1u_D1[p, i, j, k] * Erin[p, i, j, k] 
    ...
    # Interior
    E2u[p, 1:-1, 1:-1, 1:-1] = h22u_D2[p, 1:-1, 1:-1, 1:-1] * E2in[p, 1:-1, 1:-1, 1:-1] \
                             + 0.25 * h12u_D2[p, 1:-1, 1:-1, 1:-1] * (E1in[p, 1:-1, 1:-2, 1:] + N.roll(N.roll(E1in, -1, axis = 2), 1, axis = 3)[p, 1:-1, 1:-2, 1:] \
                             + N.roll(E1in, -1, axis = 2)[p, 1:-1, 1:-2, 1:] + N.roll(E1in, 1, axis = 3)[p, 1:-1, 1:-2, 1:]) \
                             + 0.25 * hr2u_D2[p, 1:-1, 1:-1, 1:-1] * (Erin[p, 1:-2, 1:-1, 1:] + N.roll(N.roll(Erin, -1, axis = 1), 1, axis = 3)[p, 1:-2, 1:-1, 1:] \
                             + N.roll(Erin, -1, axis = 1)[p, 1:-2, 1:-1, 1:] + N.roll(Erin, 1, axis = 3)[p, 1:-2, 1:-1, 1:])

    # Left/right face
    for j in [0, -1]:
        E2u[p, 1:-1, j, 1:-1] = h22u_D2[p, 1:-1, j, 1:-1] * E2in[p, 1:-1, j, 1:-1] \
                             + 0.5  * h12u_D2[p, 1:-1, j, 1:-1] * (E1in[p, 1:-1, j, 1:] + N.roll(E1in, 1, axis = 3)[p, 1:-1, j, 1:]) \
                             + 0.25 * hr2u_D2[p, 1:-1, j, 1:-1] * (Erin[p, 1:-2, j, 1:] + N.roll(N.roll(Erin, -1, axis = 1), 1, axis = 3)[p, 1:-2, j, 1:] \
                             + N.roll(Erin, -1, axis = 1)[p, 1:-2, j, 1:] + N.roll(Erin, 1, axis = 3)[p, 1:-2, j, 1:])

    # Top/bottom face
    for k in [0, -1]:
        E2u[p, 1:-1, 1:-1, k] = h22u_D2[p, 1:-1, 1:-1, k] * E2in[p, 1:-1, 1:-1, k] \
                             + 0.5 * h12u_D2[p, 1:-1, 1:-1, k] * (E1in[p, 1:-1, 1:-2, k] + N.roll(E1in, -1, axis = 2)[p, 1:-1, 1:-2, k]) \
                             + 0.5 * hr2u_D2[p, 1:-1, 1:-1, k] * (Erin[p, 1:-2, 1:-1, k] + N.roll(Erin, -1, axis = 1)[p, 1:-2, 1:-1, k])
                                 
    # Back/front face
    for i in [0, -1]:
        E2u[p, i, 1:-1, 1:-1] = h22u_D2[p, i, 1:-1, 1:-1] * E2in[p, i, 1:-1, 1:-1] \
                             + 0.25 * h12u_D2[p, i, 1:-1, 1:-1] * (E1in[p, i, 1:-2, 1:] + N.roll(N.roll(E1in, -1, axis = 2), 1, axis = 3)[p, i, 1:-2, 1:] \
                             + N.roll(E1in, -1, axis = 2)[p, i, 1:-2, 1:] + N.roll(E1in, 1, axis = 3)[p, i, 1:-2, 1:]) \
                             + 0.5 * hr2u_D2[p, i, 1:-1, 1:-1] * (Erin[p, i, 1:-1, 1:] + N.roll(Erin, 1, axis = 3)[p, i, 1:-1, 1:])
    
    # Edges
    for i in [0, -1]:
        for j in [0, -1]:
            E2u[p, i, j, 1:-1] = h22u_D2[p, i, j, 1:-1] * E2in[p, i, j, 1:-1] \
                             + 0.5 * h12u_D2[p, i, j, 1:-1] * (E1in[p, i, j, 1:] + N.roll(E1in, 1, axis = 3)[p, i, j, 1:]) \
                             + 0.5 * hr2u_D2[p, i, j, 1:-1] * (Erin[p, i, j, 1:] + N.roll(Erin, 1, axis = 3)[p, i, j, 1:])

    # Top/bottom edges
    for i in [0, -1]:
        for k in [0, -1]:
            E2u[p, i, 1:-1, k] = h22u_D2[p, i, 1:-1, k] * E2in[p, i, 1:-1, k] \
                             + 0.5 * h12u_D2[p, i, 1:-1, k] * (E1in[p, i, 1:-2, k] + N.roll(E1in, -1, axis = 2)[p, i, 1:-2, k]) \
                             + hr2u_D2[p, i, 1:-1, k] * Erin[p, i, 1:-1, k]

    for j in [0, -1]:
        for k in [0, -1]:
            E2u[p, 1:-1, j, k] = h22u_D2[p, 1:-1, j, k] * E2in[p, 1:-1, j, k] \
                               + h12u_D2[p, 1:-1, j, k] * E1in[p, 1:-1, j, k] \
                               + 0.5 * hr2u_D2[p, 1:-1, j, k] * (Erin[p, 1:-2, j, k] + N.roll(Erin, -1, axis = 1)[p, 1:-2, j, k])
                               
    # Corners
    for i in [0, -1]:
        for j in [0, -1]:
            for k in [0, -1]:
                E2u[p, i, j, k] = h22u_D2[p, i, j, k] * E2in[p, i, j, k] \
                                + h12u_D2[p, i, j, k] * E1in[p, i, j, k] \
                                + hr2u_D2[p, i, j, k] * Erin[p, i, j, k] 
...
def cov_to_contra_H(p, Hrin, H1in, H2in):
    ...
    # Interior
    Hru[p, 1:-1, 1:-1, 1:-1] = hrru_Br[p, 1:-1, 1:-1, 1:-1] * Hrin[p, 1:-1, 1:-1, 1:-1] \
                                      + 0.25 * hr1u_Br[p, 1:-1, 1:-1, 1:-1] * (H1in[p, 1:-2, 1:, 1:-1] + N.roll(N.roll(H1in, -1, axis = 1), 1, axis = 2)[p, 1:-2, 1:, 1:-1]  \
                                                                                   +  N.roll(H1in, -1, axis = 1)[p, 1:-2, 1:, 1:-1] + N.roll(H1in, 1, axis = 2)[p, 1:-2, 1:, 1:-1]) \
                                      + 0.25 * hr2u_Br[p, 1:-1, 1:-1, 1:-1] * (H2in[p, 1:-2, 1:-1, 1:] + N.roll(N.roll(H2in, -1, axis = 1), 1, axis = 3)[p, 1:-2, 1:-1, 1:]  \
                                                                                   +  N.roll(H2in, -1, axis = 1)[p, 1:-2, 1:-1, 1:] + N.roll(H2in, 1, axis = 3)[p, 1:-2, 1:-1, 1:])

    # Left/right face
    for j in [0, -1]:
        Hru[p, 1:-1, j, 1:-1] = hrru_Br[p, 1:-1, j, 1:-1] * Hrin[p, 1:-1, j, 1:-1] \
                                      + 0.5  * hr1u_Br[p, 1:-1, j, 1:-1] * (H1in[p, 1:-2, j, 1:-1] +  N.roll(H1in, -1, axis = 1)[p, 1:-2, j, 1:-1]) \
                                      + 0.25 * hr2u_Br[p, 1:-1, j, 1:-1] * (H2in[p, 1:-2, j, 1:] + N.roll(N.roll(H2in, -1, axis = 1), 1, axis = 3)[p, 1:-2, j, 1:]  \
                                                                                   +  N.roll(H2in, -1, axis = 1)[p, 1:-2, j, 1:] + N.roll(H2in, 1, axis = 3)[p, 1:-2, j, 1:])
                                      
    # Top/bottom face
    for k in [0, -1]:
        Hru[p, 1:-1, 1:-1, k] = hrru_Br[p, 1:-1, 1:-1, k] * Hrin[p, 1:-1, 1:-1, k] \
                                      + 0.25 * hr1u_Br[p, 1:-1, 1:-1, k] * (H1in[p, 1:-2, 1:, k] + N.roll(N.roll(H1in, -1, axis = 1), 1, axis = 2)[p, 1:-2, 1:, k]  \
                                                                                   +  N.roll(H1in, -1, axis = 1)[p, 1:-2, 1:, k] + N.roll(H1in, 1, axis = 2)[p, 1:-2, 1:, k]) \
                                      + 0.5 * hr2u_Br[p, 1:-1, 1:-1, k] * (H2in[p, 1:-2, 1:-1, k] +  N.roll(H2in, -1, axis = 1)[p, 1:-2, 1:-1, k])
                                 
    # Back/front face
    for i in [0, -1]:
        Hru[p, i, 1:-1, 1:-1] = hrru_Br[p, i, 1:-1, 1:-1] * Hrin[p, i, 1:-1, 1:-1] \
                                      + 0.5 * hr1u_Br[p, i, 1:-1, 1:-1] * (H1in[p, i, 1:, 1:-1] + N.roll(H1in, 1, axis = 2)[p, i, 1:, 1:-1]) \
                                      + 0.5 * hr2u_Br[p, i, 1:-1, 1:-1] * (H2in[p, i, 1:-1, 1:] + N.roll(H2in, 1, axis = 3)[p, i, 1:-1, 1:])
    
    # Edges
    for i in [0, -1]:
        for j in [0, -1]:
            Hru[p, i, j, 1:-1] = hrru_Br[p, i, j, 1:-1] * Hrin[p, i, j, 1:-1] \
                                      + hr1u_Br[p, i, j, 1:-1] * H1in[p, i, j, 1:-1] \
                                      + 0.5 * hr2u_Br[p, i, j, 1:-1] * (H2in[p, i, j, 1:] + N.roll(H2in, 1, axis = 3)[p, i, j, 1:])

    for i in [0, -1]:
        for k in [0, -1]:
            Hru[p, i, 1:-1, k] = hrru_Br[p, i, 1:-1, k] * Hrin[p, i, 1:-1, k] \
                                      + 0.5 * hr1u_Br[p, i, 1:-1, k] * (H1in[p, i, 1:, k] + N.roll(H1in, 1, axis = 2)[p, i, 1:, k]) \
                                      + hr2u_Br[p, i, 1:-1, k] * H2in[p, i, 1:-1, k]

    for j in [0, -1]:
        for k in [0, -1]:
            Hru[p, 1:-1, j, k] = hrru_Br[p, 1:-1, j, k] * Hrin[p, 1:-1, j, k] \
                                      + 0.5 * hr1u_Br[p, 1:-1, j, k] * (H1in[p, 1:-2, j, k] + N.roll(H1in, -1, axis = 1)[p, 1:-2, j, k]) \
                                      + 0.5 * hr2u_Br[p, 1:-1, j, k] * (H2in[p, 1:-2, j, k] + N.roll(H2in, -1, axis = 1)[p, 1:-2, j, k])
    ...
    for i in [0, -1]:
        for j in [0, -1]:
            for k in [0, -1]:
                Hru[p, i, j, k] = hrru_Br[p, i, j, k] * Hrin[p, i, j, k] \
                                + hr1u_Br[p, i, j, k] * H1in[p, i, j, k] \
                                + hr2u_Br[p, i, j, k] * H2in[p, i, j, k] 
    ...
    # Interior
    H1u[p, 1:-1, 1:-1, 1:-1] = h11u_B1[p, 1:-1, 1:-1, 1:-1] * H1in[p, 1:-1, 1:-1, 1:-1] \
                             + 0.25 * h12u_B1[p, 1:-1, 1:-1, 1:-1] * (H2in[p, 1:-1, 1:-2, 1:] + N.roll(N.roll(H2in, -1, axis = 2), 1, axis = 3)[p, 1:-1, 1:-2, 1:] \
                                                                                    +  N.roll(H2in, -1, axis = 2)[p, 1:-1, 1:-2, 1:] + N.roll(H2in, 1, axis = 3)[p, 1:-1, 1:-2, 1:]) \
                             + 0.25 * hr1u_B1[p, 1:-1, 1:-1, 1:-1] * (Hrin[p, 1:, 1:-2, 1:-1] + N.roll(N.roll(Hrin, 1, axis = 1), -1, axis = 2)[p, 1:, 1:-2, 1:-1] \
                                                                                    +  N.roll(Hrin, 1, axis = 1)[p, 1:, 1:-2, 1:-1] + N.roll(Hrin, -1, axis = 2)[p, 1:, 1:-2, 1:-1])
    
    # Left/right face
    for j in [0, -1]:
        H1u[p, 1:-1, j, 1:-1] = h11u_B1[p, 1:-1, j, 1:-1] * H1in[p, 1:-1, j, 1:-1] \
                             + 0.5 * h12u_B1[p, 1:-1, j, 1:-1] * (H2in[p, 1:-1, j, 1:] + N.roll(H2in, 1, axis = 3)[p, 1:-1, j, 1:]) \
                             + 0.5 * hr1u_B1[p, 1:-1, j, 1:-1] * (Hrin[p, 1:, j, 1:-1] + N.roll(Hrin, 1, axis = 1)[p, 1:, j, 1:-1])
                                      
    # Top/bottom face
    for k in [0, -1]:
        H1u[p, 1:-1, 1:-1, k] = h11u_B1[p, 1:-1, 1:-1, k] * H1in[p, 1:-1, 1:-1, k] \
                             + 0.5 * h12u_B1[p, 1:-1, 1:-1, k] * (H2in[p, 1:-1, 1:-2, k] +  N.roll(H2in, -1, axis = 2)[p, 1:-1, 1:-2, k]) \
                             + 0.25 * hr1u_B1[p, 1:-1, 1:-1, k] * (Hrin[p, 1:, 1:-2, k] + N.roll(N.roll(Hrin, 1, axis = 1), -1, axis = 2)[p, 1:, 1:-2, k] \
                                                                                    +  N.roll(Hrin, 1, axis = 1)[p, 1:, 1:-2, k] + N.roll(Hrin, -1, axis = 2)[p, 1:, 1:-2, k])
                             
    # Back/front face
    for i in [0, -1]:
        H1u[p, i, 1:-1, 1:-1] = h11u_B1[p, i, 1:-1, 1:-1] * H1in[p, i, 1:-1, 1:-1] \
                             + 0.25 * h12u_B1[p, i, 1:-1, 1:-1] * (H2in[p, i, 1:-2, 1:] + N.roll(N.roll(H2in, -1, axis = 2), 1, axis = 3)[p, i, 1:-2, 1:] \
                                                                                    +  N.roll(H2in, -1, axis = 2)[p, i, 1:-2, 1:] + N.roll(H2in, 1, axis = 3)[p, i, 1:-2, 1:]) \
                             + 0.5 * hr1u_B1[p, i, 1:-1, 1:-1] * (Hrin[p, i, 1:-2, 1:-1] + N.roll(Hrin, -1, axis = 2)[p, i, 1:-2, 1:-1])
    
    # Edges
    for i in [0, -1]:
        for j in [0, -1]:
            H1u[p, i, j, 1:-1] = h11u_B1[p, i, j, 1:-1] * H1in[p, i, j, 1:-1] \
                             + 0.5 * h12u_B1[p, i, j, 1:-1] * (H2in[p, i, j, 1:] + N.roll(H2in, 1, axis = 3)[p, i, j, 1:]) \
                             + hr1u_B1[p, i, j, 1:-1] * Hrin[p, i, j, 1:-1]

    for i in [0, -1]:
        for k in [0, -1]:
            H1u[p, i, 1:-1, k] = h11u_B1[p, i, 1:-1, k] * H1in[p, i, 1:-1, k] \
                             + 0.5 * h12u_B1[p, i, 1:-1, k] * (H2in[p, i, 1:-2, k] + N.roll(H2in, -1, axis = 2)[p, i, 1:-2, k]) \
                             + 0.5 * hr1u_B1[p, i, 1:-1, k] * (Hrin[p, i, 1:-2, k] + N.roll(Hrin, -1, axis = 2)[p, i, 1:-2, k])

    for j in [0, -1]:
        for k in [0, -1]:
            H1u[p, 1:-1, j, k] = h11u_B1[p, 1:-1, j, k] * H1in[p, 1:-1, j, k] \
                             + h12u_B1[p, 1:-1, j, k] * H2in[p, 1:-1, j, k] \
                             + 0.5 * hr1u_B1[p, 1:-1, j, k] * (Hrin[p, 1:, j, k] + N.roll(Hrin, 1, axis = 1)[p, 1:, j, k])
                               
    # Corners
    for i in [0, -1]:
        for j in [0, -1]:
            for k in [0, -1]:
                H1u[p, i, j, k] = h11u_B1[p, i, j, k] * H1in[p, i, j, k] \
                                + h12u_B1[p, i, j, k] * H2in[p, i, j, k] \
                                + hr1u_B1[p, i, j, k] * Hrin[p, i, j, k] 
    ...
    # Interior
    H2u[p, 1:-1, 1:-1, 1:-1] = h22u_B2[p, 1:-1, 1:-1, 1:-1] * H2in[p, 1:-1, 1:-1, 1:-1] \
                             + 0.25 * h12u_B2[p, 1:-1, 1:-1, 1:-1] * (H1in[p, 1:-1, 1:, 1:-2] + N.roll(N.roll(H1in, 1, axis = 2), -1, axis = 3)[p, 1:-1, 1:, 1:-2] \
                                                                                    +  N.roll(H1in, 1, axis = 2)[p, 1:-1, 1:, 1:-2] + N.roll(H1in, -1, axis = 3)[p, 1:-1, 1:, 1:-2]) \
                             + 0.25 * hr2u_B2[p, 1:-1, 1:-1, 1:-1] * (Hrin[p, 1:, 1:-1, 1:-2] + N.roll(N.roll(Hrin, 1, axis = 1), -1, axis = 3)[p, 1:, 1:-1, 1:-2] \
                                                                                    +  N.roll(Hrin, 1, axis = 1)[p, 1:, 1:-1, 1:-2] + N.roll(Hrin, -1, axis = 3)[p, 1:, 1:-1, 1:-2])
                             
    # Left/right face
    for j in [0, -1]:
        H2u[p, 1:-1, j, 1:-1] = h22u_B2[p, 1:-1, j, 1:-1] * H2in[p, 1:-1, j, 1:-1] \
                              + 0.5  * h12u_B2[p, 1:-1, j, 1:-1] * (H1in[p, 1:-1, j, 1:-2] + N.roll(H1in, -1, axis = 3)[p, 1:-1, j, 1:-2]) \
                              + 0.25 * hr2u_B2[p, 1:-1, j, 1:-1] * (Hrin[p, 1:, j, 1:-2] + N.roll(N.roll(Hrin, 1, axis = 1), -1, axis = 3)[p, 1:, j, 1:-2] \
                                                                                    +  N.roll(Hrin, 1, axis = 1)[p, 1:, j, 1:-2] + N.roll(Hrin, -1, axis = 3)[p, 1:, j, 1:-2])

    # Top/bottom face
    for k in [0, -1]:
        H2u[p, 1:-1, 1:-1, k] = h22u_B2[p, 1:-1, 1:-1, k] * H2in[p, 1:-1, 1:-1, k] \
                              + 0.5 * h12u_B2[p, 1:-1, 1:-1, k] * (H1in[p, 1:-1, 1:, k] +  N.roll(H1in, 1, axis = 2)[p, 1:-1, 1:, k]) \
                              + 0.5 * hr2u_B2[p, 1:-1, 1:-1, k] * (Hrin[p, 1:, 1:-1, k] +  N.roll(Hrin, 1, axis = 1)[p, 1:, 1:-1, k])
                             
    # Back/front face
    for i in [0, -1]:
        H2u[p, i, 1:-1, 1:-1] = h22u_B2[p, i, 1:-1, 1:-1] * H2in[p, i, 1:-1, 1:-1] \
                              + 0.25 * h12u_B2[p, i, 1:-1, 1:-1] * (H1in[p, i, 1:, 1:-2] + N.roll(N.roll(H1in, 1, axis = 2), -1, axis = 3)[p, i, 1:, 1:-2] \
                                                                                    +  N.roll(H1in, 1, axis = 2)[p, i, 1:, 1:-2] + N.roll(H1in, -1, axis = 3)[p, i, 1:, 1:-2]) \
                              + 0.5 * hr2u_B2[p, i, 1:-1, 1:-1] * (Hrin[p, i, 1:-1, 1:-2] + N.roll(Hrin, -1, axis = 3)[p, i, 1:-1, 1:-2])
    
    # Edges
    for i in [0, -1]:
        for j in [0, -1]:
            H2u[p, i, j, 1:-1] = h22u_B2[p, i, j, 1:-1] * H2in[p, i, j, 1:-1] \
                              + 0.5 * h12u_B2[p, i, j, 1:-1] * (H1in[p, i, j, 1:-2] + N.roll(H1in, -1, axis = 3)[p, i, j, 1:-2]) \
                              + 0.5 * hr2u_B2[p, i, j, 1:-1] * (Hrin[p, i, j, 1:-2] + N.roll(Hrin, -1, axis = 3)[p, i, j, 1:-2])

    for i in [0, -1]:
        for k in [0, -1]:
            H2u[p, i, 1:-1, k] = h22u_B2[p, i, 1:-1, k] * H2in[p, i, 1:-1, k] \
                              + 0.5 * h12u_B2[p, i, 1:-1, k] * (H1in[p, i, 1:, k] +  N.roll(H1in, 1, axis = 2)[p, i, 1:, k]) \
                              + hr2u_B2[p, i, 1:-1, k] * Hrin[p, i, 1:-1, k]

    for j in [0, -1]:
        for k in [0, -1]:
            H2u[p, 1:-1, j, k] = h22u_B2[p, 1:-1, j, k] * H2in[p, 1:-1, j, k] \
                               + h12u_B2[p, 1:-1, j, k] * H1in[p, 1:-1, j, k] \
                               + 0.5 * hr2u_B2[p, 1:-1, j, k] * (Hrin[p, 1:, j, k] +  N.roll(Hrin, 1, axis = 1)[p, 1:, j, k])
                        
    # Corners
    for i in [0, -1]:
        for j in [0, -1]:
            for k in [0, -1]:
                H2u[p, i, j, k] = h22u_B2[p, i, j, k] * H2in[p, i, j, k] \
                                + h12u_B2[p, i, j, k] * H1in[p, i, j, k] \
                                + hr2u_B2[p, i, j, k] * Hrin[p, i, j, k] 
...
i_abs = 4 # Thickness of absorbing layer in number of cells
