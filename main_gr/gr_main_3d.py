...
sqrt_det_h = N.empty((n_patches, Nr, Nxi_int, Neta_int, 7))

hrru = N.empty((n_patches, Nr, Nxi_int, Neta_int))
hr1u = N.empty((n_patches, Nr, Nxi_int, Neta_int))
hr2u = N.empty((n_patches, Nr, Nxi_int, Neta_int))
h11u = N.empty((n_patches, Nr, Nxi_int, Neta_int))
h12u = N.empty((n_patches, Nr, Nxi_int, Neta_int))
h22u = N.empty((n_patches, Nr, Nxi_int, Neta_int))
...
h12u_half = N.empty((n_patches, Nr, Nxi_half, 4))
h11u_half = N.empty((n_patches, Nr, Nxi_half, 4))
h22u_half = N.empty((n_patches, Nr, Nxi_half, 4))
hrru_half = N.empty((n_patches, Nr, Nxi_half, 4))
hr1u_half = N.empty((n_patches, Nr, Nxi_half, 4))
hr2u_half = N.empty((n_patches, Nr, Nxi_half, 4))
alpha_half = N.empty((n_patches, Nr, Nxi_half, 4))
...
h12u_int = N.empty((n_patches, Nr, Nxi_int, 4))
h11u_int = N.empty((n_patches, Nr, Nxi_int, 4))
h22u_int = N.empty((n_patches, Nr, Nxi_int, 4))
hrru_int = N.empty((n_patches, Nr, Nxi_int, 4))
hr1u_int = N.empty((n_patches, Nr, Nxi_int, 4))
hr2u_int = N.empty((n_patches, Nr, Nxi_int, 4))
alpha_int = N.empty((n_patches, Nr, Nxi_int, 4))
...
for p in range(n_patches):
    for i in range(Nxi_int):
        ...
        for j in range(Neta_int):
            ...
            # sqrt_det_h[p, :, i, j, 0] = sqrtdeth(p, r0, xi0, eta0, a)
            ...
            hrru[p, :, i, j] = inv_metric[:, 0, 0]
            hr1u[p, :, i, j] = inv_metric[:, 0, 1]
            hr2u[p, :, i, j] = inv_metric[:, 0, 2]
            ...
            metric = N.array([[hrrd[p, :, i, j, 4], hr1d[p, :, i, j, 4], hr2d[p, :, i, j, 4]], \
                              [hr1d[p, :, i, j, 4], h11d[p, :, i, j, 4], h12d[p, :, i, j, 4]], \
                              [hr2d[p, :, i, j, 4], h12d[p, :, i, j, 4], h22d[p, :, i, j, 4]]])
            ...
            metric = N.array([[hrrd[p, :, i, j, 5], hr1d[p, :, i, j, 5], hr2d[p, :, i, j, 5]], \
                              [hr1d[p, :, i, j, 5], h11d[p, :, i, j, 5], h12d[p, :, i, j, 5]], \
                              [hr2d[p, :, i, j, 5], h12d[p, :, i, j, 5], h22d[p, :, i, j, 5]]])
            ...
            metric = N.array([[hrrd[p, :, i, j, 6], hr1d[p, :, i, j, 6], hr2d[p, :, i, j, 6]], \
                              [hr1d[p, :, i, j, 6], h11d[p, :, i, j, 6], h12d[p, :, i, j, 6]], \
                              [hr2d[p, :, i, j, 6], h12d[p, :, i, j, 6], h22d[p, :, i, j, 6]]])
...
dt = cfl * N.min(1.0 / N.sqrt(hrru / (dr * dr) + h11u / (dxi * dxi) + h22u / (deta * deta))) # + 2.0 * h12u / (dxi * deta) + 2.0 * hr1u / (dr * dxi) + 2.0 * hr2u / (dr * deta)))
...
def compute_diff_H(p):
    ...
    dH1dr[p, ir1:ir2, :, :] = (H1d[p, ir1:ir2, :, :] - N.roll(H1d, 1, axis = 1)[p, ir1:ir2, :, :]) / dr
    dH2dr[p, ir1:ir2, :, :] = (H2d[p, ir1:ir2, :, :] - N.roll(H2d, 1, axis = 1)[p, ir1:ir2, :, :]) / dr
...
def compute_diff_E(p):
    ...
    dE1dr[p, ir1:ir2, :, :] = (N.roll(E1d, -1, axis = 1)[p, ir1:ir2, :, :] - E1d[p, ir1:ir2, :, :]) / dr
    dE2dr[p, ir1:ir2, :, :] = (N.roll(E2d, -1, axis = 1)[p, ir1:ir2, :, :] - E2d[p, ir1:ir2, :, :]) / dr
...
def push_D(p, Drin, D1in, D2in, dtin):
    ...
    # Interior
    D1in[p, ir1:ir2, 1:-1, :] += dtin * (dHrd2[p, ir1:ir2, 1:-1, :] - dH2dr[p, ir1:ir2, 1:-1, :]) / sqrt_det_h[p, ir1:ir2, :-1, :, 1] 
    # Left edge
    D1in[p, ir1:ir2, 0, :] += dtin * (dHrd2[p, ir1:ir2, 0, :] - dH2dr[p, ir1:ir2, 0, :]) / sqrt_det_h[p, ir1:ir2, 0, :, 0] 
    # Right edge
    D1in[p, ir1:ir2, -1, :] += dtin * (dHrd2[p, ir1:ir2, -1, :] - dH2dr[p, ir1:ir2, -1, :]) / sqrt_det_h[p, ir1:ir2, -1, :, 0]

    ir1 = NG
    ir2 = Nr0 + NG + 1    

    # Interior
    D2in[p, ir1:ir2, :, 1:-1] += dtin * (dH1dr[p, ir1:ir2, :, 1:-1] - dHrd1[p, ir1:ir2, :, 1:-1]) / sqrt_det_h[p, ir1:ir2, :, :-1, 2]
    # Bottom edge
    D2in[p, ir1:ir2, :, 0] += dtin * (dH1dr[p, ir1:ir2, :, 0] - dHrd1[p, ir1:ir2, :, 0]) / sqrt_det_h[p, ir1:ir2, :, 0, 0]
    # Top edge
    D2in[p, ir1:ir2, :, -1] += dtin * (dH1dr[p, ir1:ir2, :, -1] - dHrd1[p, ir1:ir2, :, -1]) / sqrt_det_h[p, ir1:ir2, :, -1, 0]
...
def push_B(p, Brin, B1in, B2in, dtin):
    ...
    # Interior
    B1in[p, ir1:ir2, :, 1:-1] += dtin * (dE2dr[p, ir1:ir2, :, 1:-1] - dErd2[p, ir1:ir2, :, 1:-1]) / sqrt_det_h[p, ir1:ir2, :, :-1, 6]
    # Bottom edge
    B1in[p, ir1:ir2, :, 0] += dtin * (dE2dr[p, ir1:ir2, :, 0] - dErd2[p, ir1:ir2, :, 0]) / sqrt_det_h[p, ir1:ir2, :, 0, 4]
    # Top edge
    B1in[p, ir1:ir2, :, -1] += dtin * (dE2dr[p, ir1:ir2, :, -1] - dErd2[p, ir1:ir2, :, -1]) / sqrt_det_h[p, ir1:ir2, :, -1, 4]

    ir1 = NG - 1
    ir2 = Nr0 + NG

    # Interior
    B2in[p, ir1:ir2, 1:-1, :] += dtin * (dErd1[p, ir1:ir2, 1:-1, :] - dE1dr[p, ir1:ir2, 1:-1, :]) / sqrt_det_h[p, ir1:ir2, :-1, :, 5] 
    # Left edge
    B2in[p, ir1:ir2, 0, :] += dtin * (dErd1[p, ir1:ir2, 0, :] - dE1dr[p, ir1:ir2, 0, :]) / sqrt_det_h[p, ir1:ir2, 0, :, 4] 
    # Right edge
    B2in[p, ir1:ir2, -1, :] += dtin * (dErd1[p, ir1:ir2, -1, :] - dE1dr[p, ir1:ir2, -1, :]) / sqrt_det_h[p, ir1:ir2, -1, :, 4]
...
def contra_to_cov_D(p, Drin, D1in, D2in):
    ...
    # Interior
    Drd[p, ir1:ir2, 1:-1, 1:-1] = hrrd[p, ir1:ir2, 1:-1, 1:-1, 4] * Drin[p, ir1:ir2, 1:-1, 1:-1] \
                                      + 0.25 * hr1d[p, ir1:ir2, 1:-1, 1:-1, 4] * (D1in[p, ir1:ir2, 1:-2, 1:-1] + N.roll(N.roll(D1in, -1, axis = 1), -1, axis = 2)[p, ir1:ir2, 1:-2, 1:-1]  \
                                                                                     +  N.roll(D1in, -1, axis = 1)[p, ir1:ir2, 1:-2, 1:-1] + N.roll(D1in, -1, axis = 2)[p, ir1:ir2, 1:-2, 1:-1]) \
                                      + 0.25 * hr2d[p, ir1:ir2, 1:-1, 1:-1, 4] * (D2in[p, ir1:ir2, 1:-1, 1:-2] + N.roll(N.roll(D2in, -1, axis = 1), -1, axis = 3)[p, ir1:ir2, 1:-1, 1:-2]  \
                                                                                     +  N.roll(D2in, -1, axis = 1)[p, ir1:ir2, 1:-1, 1:-2] + N.roll(D2in, -1, axis = 3)[p, ir1:ir2, 1:-1, 1:-2]) \

    # Left edge
    Drd[p, ir1:ir2, 0, 1:-1] = hrrd[p, ir1:ir2, 0, 1:-1, 4] * Drin[p, ir1:ir2, 0, 1:-1] \
                                   + 0.5  * hr1d[p, ir1:ir2, 0, 1:-1, 4] * (D1in[p, ir1:ir2, 0, 1:-1] + N.roll(D1in, -1, axis = 1)[p, ir1:ir2, 0, 1:-1]) \
                                   + 0.25 * hr2d[p, ir1:ir2, 0, 1:-1, 4] * (D2in[p, ir1:ir2, 0, 1:-2] + N.roll(N.roll(D2in, -1, axis = 1), -1, axis = 3)[p, ir1:ir2, 0, 1:-2]  \
                                                                               +  N.roll(D2in, -1, axis = 1)[p, ir1:ir2, 0, 1:-2] + N.roll(D2in, -1, axis = 3)[p, ir1:ir2, 0, 1:-2])
    # Right edge
    Drd[p, ir1:ir2, -1, 1:-1] = hrrd[p, ir1:ir2, -1, 1:-1, 4] * Drin[p, ir1:ir2, -1, 1:-1] \
                                    + 0.5  * hr1d[p, ir1:ir2, -1, 1:-1, 4] * (D1in[p, ir1:ir2, -1, 1:-1] + N.roll(D1in, -1, axis = 1)[p, ir1:ir2, -1, 1:-1]) \
                                    + 0.25 * hr2d[p, ir1:ir2, -1, 1:-1, 4] * (D2in[p, ir1:ir2, -1, 1:-2] + N.roll(N.roll(D2in, -1, axis = 1), -1, axis = 3)[p, ir1:ir2, -1, 1:-2]  \
                                                                                 +  N.roll(D2in, -1, axis = 1)[p, ir1:ir2, -1, 1:-2] + N.roll(D2in, -1, axis = 3)[p, ir1:ir2, -1, 1:-2])
    # Bottom edge
    Drd[p, ir1:ir2, 1:-1, 0] = hrrd[p, ir1:ir2, 1:-1, 0, 4] * Drin[p, ir1:ir2, 1:-1, 0] \
                                   + 0.25 * hr1d[p, ir1:ir2, 1:-1, 0, 4] * (D1in[p, ir1:ir2, 1:-2, 0] + N.roll(N.roll(D1in, -1, axis = 1), -1, axis = 2)[p, ir1:ir2, 1:-2, 0]  \
                                                                              +   N.roll(D1in, -1, axis = 1)[p, ir1:ir2, 1:-2, 0] + N.roll(D1in, -1, axis = 2)[p, ir1:ir2, 1:-2, 0]) \
                                   + 0.5  * hr2d[p, ir1:ir2, 1:-1, 0, 4] * (D2in[p, ir1:ir2, 1:-1, 0] + N.roll(D2in, -1, axis = 1)[p, ir1:ir2, 1:-1, 0])
    # Top edge
    Drd[p, ir1:ir2, 1:-1, -1] = hrrd[p, ir1:ir2, 1:-1, -1, 4] * Drin[p, ir1:ir2, 1:-1, -1] \
                                    + 0.25 * hr1d[p, ir1:ir2, 1:-1, -1, 4] * (D1in[p, ir1:ir2, 1:-2, -1] + N.roll(N.roll(D1in, -1, axis = 1), -1, axis = 2)[p, ir1:ir2, 1:-2, -1]  \
                                                                                +   N.roll(D1in, -1, axis = 1)[p, ir1:ir2, 1:-2, -1] + N.roll(D1in, -1, axis = 2)[p, ir1:ir2, 1:-2, -1]) \
                                    + 0.5  * hr2d[p, ir1:ir2, 1:-1, -1, 4] * (D2in[p, ir1:ir2, 1:-1, -1] + N.roll(D2in, -1, axis = 1)[p, ir1:ir2, 1:-1, -1])
    # Bottom-left corner
    Drd[p, ir1:ir2, 0, 0] = hrrd[p, ir1:ir2, 0, 0, 4] * Drin[p, ir1:ir2, 0, 0] \
                                + 0.5 * hr1d[p, ir1:ir2, 0, 0, 4] * (D1in[p, ir1:ir2, 0, 0] + N.roll(D1in, -1, axis = 1)[p, ir1:ir2, 0, 0]) \
                                + 0.5 * hr2d[p, ir1:ir2, 0, 0, 4] * (D2in[p, ir1:ir2, 0, 0] + N.roll(D2in, -1, axis = 1)[p, ir1:ir2, 0, 0])
    # Top-left corner
    Drd[p, ir1:ir2, 0, -1] = hrrd[p, ir1:ir2, 0, -1, 4] * Drin[p, ir1:ir2, 0, -1] \
                                 + 0.5 * hr1d[p, ir1:ir2, 0, -1, 4] * (D1in[p, ir1:ir2, 0, -1] + N.roll(D1in, -1, axis = 1)[p, ir1:ir2, 0, -1]) \
                                 + 0.5 * hr2d[p, ir1:ir2, 0, -1, 4] * (D2in[p, ir1:ir2, 0, -1] + N.roll(D2in, -1, axis = 1)[p, ir1:ir2, 0, -1])
    # Bottom-right corner
    Drd[p, ir1:ir2, -1, 0] = hrrd[p, ir1:ir2, -1, 0, 4] * Drin[p, ir1:ir2, -1, 0] \
                                 + 0.5 * hr1d[p, ir1:ir2, -1, 0, 4] * (D1in[p, ir1:ir2, -1, 0] + N.roll(D1in, -1, axis = 1)[p, ir1:ir2, -1, 0]) \
                                 + 0.5 * hr2d[p, ir1:ir2, -1, 0, 4] * (D2in[p, ir1:ir2, -1, 0] + N.roll(D2in, -1, axis = 1)[p, ir1:ir2, -1, 0])
    # Top-right corner
    Drd[p, ir1:ir2, -1, -1] = hrrd[p, ir1:ir2, -1, -1, 4] * Drin[p, ir1:ir2, -1, -1] \
                                  + 0.5 * hr1d[p, ir1:ir2, -1, -1, 4] * (D1in[p, ir1:ir2, -1, -1] + N.roll(D1in, -1, axis = 1)[p, ir1:ir2, -1, -1]) \
                                  + 0.5 * hr2d[p, ir1:ir2, -1, -1, 4] * (D2in[p, ir1:ir2, -1, -1] + N.roll(D2in, -1, axis = 1)[p, ir1:ir2, -1, -1])
    ...
    D1d[p, ir1:ir2, 1:-1, 1:-1] = h11d[p, ir1:ir2, :-1, 1:-1, 1] * D1in[p, ir1:ir2, 1:-1, 1:-1] \
                                      ...
                                      + 0.25 * hr1d[p, ir1:ir2, :-1, 1:-1, 1] * (Drin[p, ir1:ir2, 1:, 1:-1] + N.roll(N.roll(Drin, 1, axis = 1), 1, axis = 2)[p, ir1:ir2, 1:, 1:-1] \
    ...
    D1d[p, ir1:ir2, 0, 1:-1] = h11d[p, ir1:ir2, 0, 1:-1, 0] * D1in[p, ir1:ir2, 0, 1:-1] \
                                   ...
                                   + 0.5 * hr1d[p, ir1:ir2, 0, 1:-1, 0] * (Drin[p, ir1:ir2, 0, 1:-1] + N.roll(Drin, 1, axis = 1)[p, ir1:ir2, 0, 1:-1])
    # Right edge
    D1d[p, ir1:ir2, -1, 1:-1] = h11d[p, ir1:ir2, -1, 1:-1, 0] * D1in[p, ir1:ir2, -1, 1:-1] \
                                    ...
                                    + 0.5 * hr1d[p, ir1:ir2, -1, 1:-1, 0] * (Drin[p, ir1:ir2, -1, 1:-1] + N.roll(Drin, 1, axis = 1)[p, ir1:ir2, -1, 1:-1])
    # Bottom edge
    D1d[p, ir1:ir2, 1:-1, 0] = h11d[p, ir1:ir2, :-1, 0, 1] * D1in[p, ir1:ir2, 1:-1, 0] \
                                   ...
                                   + 0.25 * hr1d[p, ir1:ir2, :-1, 0, 1] * (Drin[p, ir1:ir2, 1:, 0] + N.roll(N.roll(Drin, 1, axis = 1), 1, axis = 2)[p, ir1:ir2, 1:, 0] \
    ...
    D1d[p, ir1:ir2, 1:-1, -1] = h11d[p, ir1:ir2, :-1, -1, 1] * D1in[p, ir1:ir2, 1:-1, -1] \
                                    ...
                                    + 0.25 * hr1d[p, ir1:ir2, :-1, -1, 1] * (Drin[p, ir1:ir2, 1:, -1] + N.roll(N.roll(Drin, 1, axis = 1), 1, axis = 2)[p, ir1:ir2, 1:, -1] \
    ...
    D1d[p, ir1:ir2, 0, 0] = h11d[p, ir1:ir2, 0, 0, 0] * D1in[p, ir1:ir2, 0, 0] \
                                ...
                                + 0.5 * hr1d[p, ir1:ir2, 0, 0, 0] * (Drin[p, ir1:ir2, 0, 0] + N.roll(Drin, 1, axis = 1)[p, ir1:ir2, 0, 0])
    # Top-left corner
    D1d[p, ir1:ir2, 0, -1] = h11d[p, ir1:ir2, 0, -1, 0] * D1in[p, ir1:ir2, 0, -1] \
                                 + h12d[p, ir1:ir2, 0, -1, 0] * D2in[p, ir1:ir2, 0, -1] \
                                 + 0.5 * hr1d[p, ir1:ir2, 0, -1, 0] * (Drin[p, ir1:ir2, 0, -1] + N.roll(Drin, 1, axis = 1)[p, ir1:ir2, 0, -1])
    # Bottom-right corner
    D1d[p, ir1:ir2, -1, 0] = h11d[p, ir1:ir2, -1, 0, 0] * D1in[p, ir1:ir2, -1, 0] \
                                 + h12d[p, ir1:ir2, -1, 0, 0] * D2in[p, ir1:ir2, -1, 0] \
                                 + 0.5 * hr1d[p, ir1:ir2, -1, 0, 0] * (Drin[p, ir1:ir2, -1, 0] + N.roll(Drin, 1, axis = 1)[p, ir1:ir2, -1, 0])
    # Top-right corner
    D1d[p, ir1:ir2, -1, -1] = h11d[p, ir1:ir2, -1, -1, 0] * D1in[p, ir1:ir2, -1, -1] \
                                  + h12d[p, ir1:ir2, -1, -1, 0] * D2in[p, ir1:ir2, -1, -1] \
                                  + 0.5 * hr1d[p, ir1:ir2, -1, -1, 0] * (Drin[p, ir1:ir2, -1, -1] + N.roll(Drin, 1, axis = 1)[p, ir1:ir2, -1, -1])
    ...
    D2d[p, ir1:ir2, 1:-1, 1:-1] = h22d[p, ir1:ir2, 1:-1, :-1, 2] * D2in[p, ir1:ir2, 1:-1, 1:-1] \
                          ...
                          + 0.25 * hr2d[p, ir1:ir2, 1:-1, :-1, 2] * (Drin[p, ir1:ir2, 1:-1, 1:] + N.roll(N.roll(Drin, 1, axis = 1), 1, axis = 3)[p, ir1:ir2, 1:-1, 1:] \
    ...
    D2d[p, ir1:ir2, 0, 1:-1] = h22d[p, ir1:ir2, 0, :-1, 2] * D2in[p, ir1:ir2, 0, 1:-1] \
                       ...
                       + 0.25 * hr2d[p, ir1:ir2, 0, :-1, 2] * (Drin[p, ir1:ir2, 0, 1:] + N.roll(N.roll(Drin, 1, axis = 1), 1, axis = 3)[p, ir1:ir2, 0, 1:] \
    ...
    D2d[p, ir1:ir2, -1, 1:-1] = h22d[p, ir1:ir2, -1, :-1, 2] * D2in[p, ir1:ir2, -1, 1:-1] \
                        ...
                        + 0.25 * hr2d[p, ir1:ir2, -1, :-1, 2] * (Drin[p, ir1:ir2, -1, 1:] + N.roll(N.roll(Drin, 1, axis = 1), 1, axis = 3)[p, ir1:ir2, -1, 1:] \
    ...
    D2d[p, ir1:ir2, 1:-1, 0] = h22d[p, ir1:ir2, 1:-1, 0, 0] * D2in[p, ir1:ir2, 1:-1, 0] \
                       ...
                       + 0.5 * hr2d[p, ir1:ir2, 1:-1, 0, 0] * (Drin[p, ir1:ir2, 1:-1, 0] + N.roll(Drin, 1, axis = 1)[p, ir1:ir2, 1:-1, 0])
    # Top edge
    D2d[p, ir1:ir2, 1:-1, -1] = h22d[p, ir1:ir2, 1:-1, -1, 0] * D2in[p, ir1:ir2, 1:-1, -1] \
                        ...
                        + 0.5 * hr2d[p, ir1:ir2, 1:-1, -1, 0] * (Drin[p, ir1:ir2, 1:-1, -1] + N.roll(Drin, 1, axis = 1)[p, ir1:ir2, 1:-1, -1])
    # Bottom-left corner
    D2d[p, ir1:ir2, 0, 0] = h22d[p, ir1:ir2, 0, 0, 0] * D2in[p, ir1:ir2, 0, 0] \
                                + h12d[p, ir1:ir2, 0, 0, 0] * D1in[p, ir1:ir2, 0, 0] \
                                + 0.5 * hr2d[p, ir1:ir2, 0, 0, 0] * (Drin[p, ir1:ir2, 0, 0] + N.roll(Drin, 1, axis = 1)[p, ir1:ir2, 0, 0])
    # Top-left corner
    D2d[p, ir1:ir2, 0, -1] = h22d[p, ir1:ir2, 0, -1, 0] * D2in[p, ir1:ir2, 0, -1] \
                                 + h12d[p, ir1:ir2, 0, -1, 0] * D1in[p, ir1:ir2, 0, -1] \
                                 + 0.5 * hr2d[p, ir1:ir2, 0, -1, 0] * (Drin[p, ir1:ir2, 0, -1] + N.roll(Drin, 1, axis = 1)[p, ir1:ir2, 0, -1])
    # Bottom-right corner
    D2d[p, ir1:ir2, -1, 0] = h22d[p, ir1:ir2, -1, 0, 0] * D2in[p, ir1:ir2, -1, 0] \
                                 + h12d[p, ir1:ir2, -1, 0, 0] * D1in[p, ir1:ir2, -1, 0] \
                                 + 0.5 * hr2d[p, ir1:ir2, -1, 0, 0] * (Drin[p, ir1:ir2, -1, 0] + N.roll(Drin, 1, axis = 1)[p, ir1:ir2, -1, 0])
    # Top-right corner
    D2d[p, ir1:ir2, -1, -1] = h22d[p, ir1:ir2, -1, -1, 0] * D2in[p, ir1:ir2, -1, -1] \
                                  + h12d[p, ir1:ir2, -1, -1, 0] * D1in[p, ir1:ir2, -1, -1] \
                                  + 0.5 * hr2d[p, ir1:ir2, -1, -1, 0] * (Drin[p, ir1:ir2, -1, -1] + N.roll(Drin, 1, axis = 1)[p, ir1:ir2, -1, -1])
...
def contra_to_cov_B(p, Brin, B1in, B2in):
    ...
    # Interior
    Brd[p, ir1:ir2, 1:-1, 1:-1] = hrrd[p, ir1:ir2, :-1, :-1, 3] * Brin[p, ir1:ir2, 1:-1, 1:-1] \
                                      + 0.25 * hr1d[p, ir1:ir2, :-1, :-1, 3] * (B1in[p, ir1:ir2, 1:, 1:-1] + N.roll(N.roll(B1in, 1, axis = 1), 1, axis = 2)[p, ir1:ir2, 1:, 1:-1]  \
                                                                                   +  N.roll(B1in, 1, axis = 1)[p, ir1:ir2, 1:, 1:-1] + N.roll(B1in, 1, axis = 2)[p, ir1:ir2, 1:, 1:-1]) \
                                      + 0.25 * hr2d[p, ir1:ir2, :-1, :-1, 3] * (B2in[p, ir1:ir2, 1:-1, 1:] + N.roll(N.roll(B2in, 1, axis = 1), 1, axis = 3)[p, ir1:ir2, 1:-1, 1:]  \
                                                                                   +  N.roll(B2in, 1, axis = 1)[p, ir1:ir2, 1:-1, 1:] + N.roll(B2in, 1, axis = 3)[p, ir1:ir2, 1:-1, 1:])

    # Left edge
    Brd[p, ir1:ir2, 0, 1:-1] = hrrd[p, ir1:ir2, 0, :-1, 2] * Brin[p, ir1:ir2, 0, 1:-1] \
                                   + 0.5  * hr1d[p, ir1:ir2, 0, :-1, 2] * (B1in[p, ir1:ir2, 0, 1:-1] +  N.roll(B1in, 1, axis = 1)[p, ir1:ir2, 0, 1:-1]) \
                                   + 0.25 * hr2d[p, ir1:ir2, 0, :-1, 2] * (B2in[p, ir1:ir2, 0, 1:] + N.roll(N.roll(B2in, 1, axis = 1), 1, axis = 3)[p, ir1:ir2, 0, 1:]  \
                                                                              +  N.roll(B2in, 1, axis = 1)[p, ir1:ir2, 0, 1:] + N.roll(B2in, 1, axis = 3)[p, ir1:ir2, 0, 1:])

    # Right edge
    Brd[p, ir1:ir2, -1, 1:-1] = hrrd[p, ir1:ir2, -1, :-1, 2] * Brin[p, ir1:ir2, -1, 1:-1] \
                                    + 0.5  * hr1d[p, ir1:ir2, -1, :-1, 2] * (B1in[p, ir1:ir2, -1, 1:-1] +  N.roll(B1in, 1, axis = 1)[p, ir1:ir2, -1, 1:-1]) \
                                    + 0.25 * hr2d[p, ir1:ir2, -1, :-1, 2] * (B2in[p, ir1:ir2, -1, 1:] + N.roll(N.roll(B2in, 1, axis = 1), 1, axis = 3)[p, ir1:ir2, -1, 1:]  \
                                                                                +  N.roll(B2in, 1, axis = 1)[p, ir1:ir2, -1, 1:] + N.roll(B2in, 1, axis = 3)[p, ir1:ir2, -1, 1:])
    
    # Bottom edge
    Brd[p, ir1:ir2, 1:-1, 0] = hrrd[p, ir1:ir2, :-1, 0, 1] * Brin[p, ir1:ir2, 1:-1, 0] \
                                   + 0.25 * hr1d[p, ir1:ir2, :-1, 0, 1] * (B1in[p, ir1:ir2, 1:, 0] + N.roll(N.roll(B1in, 1, axis = 1), 1, axis = 2)[p, ir1:ir2, 1:, 0]  \
                                                                              +  N.roll(B1in, 1, axis = 1)[p, ir1:ir2, 1:, 0] + N.roll(B1in, 1, axis = 2)[p, ir1:ir2, 1:, 0]) \
                                   + 0.5  * hr2d[p, ir1:ir2, :-1, 0, 1] * (B2in[p, ir1:ir2, 1:-1, 0] +  N.roll(B2in, 1, axis = 1)[p, ir1:ir2, 1:-1, 0])
    # Top edge
    Brd[p, ir1:ir2, 1:-1, -1] = hrrd[p, ir1:ir2, :-1, -1, 1] * Brin[p, ir1:ir2, 1:-1, -1] \
                                    + 0.25 * hr1d[p, ir1:ir2, :-1, -1, 1] * (B1in[p, ir1:ir2, 1:, -1] + N.roll(N.roll(B1in, 1, axis = 1), 1, axis = 2)[p, ir1:ir2, 1:, -1]  \
                                                                                +  N.roll(B1in, 1, axis = 1)[p, ir1:ir2, 1:, -1] + N.roll(B1in, 1, axis = 2)[p, ir1:ir2, 1:, -1]) \
                                    + 0.5  * hr2d[p, ir1:ir2, :-1, -1, 1] * (B2in[p, ir1:ir2, 1:-1, -1] +  N.roll(B2in, 1, axis = 1)[p, ir1:ir2, 1:-1, -1])
                                      
    # Bottom-left corner
    Brd[p, ir1:ir2, 0, 0] = hrrd[p, ir1:ir2, 0, 0, 0] * Brin[p, ir1:ir2, 0, 0] \
                                + 0.5 * hr1d[p, ir1:ir2, 0, 0, 0] * (B1in[p, ir1:ir2, 0, 0] + N.roll(B1in, 1, axis = 1)[p, ir1:ir2, 0, 0]) \
                                + 0.5 * hr2d[p, ir1:ir2, 0, 0, 0] * (B2in[p, ir1:ir2, 0, 0] + N.roll(B2in, 1, axis = 1)[p, ir1:ir2, 0, 0])
    # Top-left corner
    Brd[p, ir1:ir2, 0, -1] = hrrd[p, ir1:ir2, 0, -1, 0] * Brin[p, ir1:ir2, 0, -1] \
                                 + 0.5 * hr1d[p, ir1:ir2, 0, -1, 0] * (B1in[p, ir1:ir2, 0, -1] + N.roll(B1in, 1, axis = 1)[p, ir1:ir2, 0, -1]) \
                                 + 0.5 * hr2d[p, ir1:ir2, 0, -1, 0] * (B2in[p, ir1:ir2, 0, -1] + N.roll(B2in, 1, axis = 1)[p, ir1:ir2, 0, -1])
    # Bottom-right corner
    Brd[p, ir1:ir2, -1, 0] = hrrd[p, ir1:ir2, -1, 0, 0] * Brin[p, ir1:ir2, -1, 0] \
                                 + 0.5 * hr1d[p, ir1:ir2, -1, 0, 0] * (B1in[p, ir1:ir2, -1, 0] + N.roll(B1in, 1, axis = 1)[p, ir1:ir2, -1, 0]) \
                                 + 0.5 * hr2d[p, ir1:ir2, -1, 0, 0] * (B2in[p, ir1:ir2, -1, 0] + N.roll(B2in, 1, axis = 1)[p, ir1:ir2, -1, 0])
    # Top-right corner
    Brd[p, ir1:ir2, -1, -1] = hrrd[p, ir1:ir2, -1, -1, 0] * Brin[p, ir1:ir2, -1, -1] \
                                  + 0.5 * hr1d[p, ir1:ir2, -1, -1, 0] * (B1in[p, ir1:ir2, -1, -1] + N.roll(B1in, 1, axis = 1)[p, ir1:ir2, -1, -1]) \
                                  + 0.5 * hr2d[p, ir1:ir2, -1, -1, 0] * (B2in[p, ir1:ir2, -1, -1] + N.roll(B2in, 1, axis = 1)[p, ir1:ir2, -1, -1])
    ...
    B1d[p, ir1:ir2, 1:-1, 1:-1] = h11d[p, ir1:ir2, 1:-1, :-1, 6] * B1in[p, ir1:ir2, 1:-1, 1:-1] \
                                      ...
                                      + 0.25 * hr1d[p, ir1:ir2, 1:-1, :-1, 6] * (Brin[p, ir1:ir2, 1:-2, 1:-1] + N.roll(N.roll(Brin, -1, axis = 1), -1, axis = 2)[p, ir1:ir2, 1:-2, 1:-1] \
    ...
    B1d[p, ir1:ir2, 0, 1:-1] = h11d[p, ir1:ir2, 0, :-1, 6] * B1in[p, ir1:ir2, 0, 1:-1] \
                                   ...
                                   + 0.5 * hr1d[p, ir1:ir2, 0, :-1, 6] * (Brin[p, ir1:ir2, 0, 1:-1] + N.roll(Brin, -1, axis = 1)[p, ir1:ir2, 0, 1:-1])
    # Right edge
    B1d[p, ir1:ir2, -1, 1:-1] = h11d[p, ir1:ir2, -1, :-1, 6] * B1in[p, ir1:ir2, -1, 1:-1] \
                                   ...
                                   + 0.5 * hr1d[p, ir1:ir2, -1, :-1, 6] * (Brin[p, ir1:ir2, -1, 1:-1] + N.roll(Brin, -1, axis = 1)[p, ir1:ir2, -1, 1:-1])
    # Bottom edge
    B1d[p, ir1:ir2, 1:-1, 0] = h11d[p, ir1:ir2, 1:-1, 0, 4] * B1in[p, ir1:ir2, 1:-1, 0] \
                                   ...
                                   + 0.25 * hr1d[p, ir1:ir2, 1:-1, 0, 4] * (Brin[p, ir1:ir2, 1:-2, 0] + N.roll(N.roll(Brin, -1, axis = 1), -1, axis = 2)[p, ir1:ir2, 1:-2, 0] \
    ...
    B1d[p, ir1:ir2, 1:-1, -1] = h11d[p, ir1:ir2, 1:-1, -1, 4] * B1in[p, ir1:ir2, 1:-1, -1] \
                                    ...
                                    + 0.25 * hr1d[p, ir1:ir2, 1:-1, -1, 4] * (Brin[p, ir1:ir2, 1:-2, -1] + N.roll(N.roll(Brin, -1, axis = 1), -1, axis = 2)[p, ir1:ir2, 1:-2, -1] \
    ...
    B1d[p, ir1:ir2, 0, 0] = h11d[p, ir1:ir2, 0, 0, 4] * B1in[p, ir1:ir2, 0, 0] \
                                ...
                                + 0.5 * hr1d[p, ir1:ir2, 0, 0, 4] * (Brin[p, ir1:ir2, 0, 0] + N.roll(Brin, -1, axis = 1)[p, ir1:ir2, 0, 0])
    # Top-left corner
    B1d[p, ir1:ir2, 0, -1] = h11d[p, ir1:ir2, 0, -1, 4] * B1in[p, ir1:ir2, 0, -1] \
                                 + h12d[p, ir1:ir2, 0, -1, 4] * B2in[p, ir1:ir2, 0, -1] \
                                 + 0.5 * hr1d[p, ir1:ir2, 0, -1, 4] * (Brin[p, ir1:ir2, 0, -1] + N.roll(Brin, -1, axis = 1)[p, ir1:ir2, 0, -1])
    # Bottom-right corner
    B1d[p, ir1:ir2, -1, 0] = h11d[p, ir1:ir2, -1, 0, 4] * B1in[p, ir1:ir2, -1, 0] \
                                 + h12d[p, ir1:ir2, -1, 0, 4] * B2in[p, ir1:ir2, -1, 0] \
                                 + 0.5 * hr1d[p, ir1:ir2, -1, 0, 4] * (Brin[p, ir1:ir2, -1, 0] + N.roll(Brin, -1, axis = 1)[p, ir1:ir2, -1, 0])
    # Top-right corner
    B1d[p, ir1:ir2, -1, -1] = h11d[p, ir1:ir2, -1, -1, 4] * B1in[p, ir1:ir2, -1, -1] \
                                  + h12d[p, ir1:ir2, -1, -1, 4] * B2in[p, ir1:ir2, -1, -1] \
                                  + 0.5 * hr1d[p, ir1:ir2, -1, -1, 4] * (Brin[p, ir1:ir2, -1, -1] + N.roll(Brin, -1, axis = 1)[p, ir1:ir2, -1, -1])
    ...
    B2d[p, ir1:ir2, 1:-1, 1:-1] = h22d[p, ir1:ir2, :-1, 1:-1, 5] * B2in[p, ir1:ir2, 1:-1, 1:-1] \
                                      ...
                                      + 0.25 * hr2d[p, ir1:ir2, :-1, 1:-1, 5] * (Brin[p, ir1:ir2, 1:-1, 1:-2] + N.roll(N.roll(Brin, -1, axis = 1), -1, axis = 3)[p, ir1:ir2, 1:-1, 1:-2] \
    ...
    B2d[p, ir1:ir2, 0, 1:-1] = h22d[p, ir1:ir2, 0, 1:-1, 4] * B2in[p, ir1:ir2, 0, 1:-1] \
                                   ...
                                   + 0.25 * hr2d[p, ir1:ir2, 0, 1:-1, 4] * (Brin[p, ir1:ir2, 0, 1:-2] + N.roll(N.roll(Brin, -1, axis = 1), -1, axis = 3)[p, ir1:ir2, 0, 1:-2] \
    ...
    B2d[p, ir1:ir2, -1, 1:-1] = h22d[p, ir1:ir2, -1, 1:-1, 4] * B2in[p, ir1:ir2, -1, 1:-1] \
                                    ...
                                    + 0.25 * hr2d[p, ir1:ir2, -1, 1:-1, 4] * (Brin[p, ir1:ir2, -1, 1:-2] + N.roll(N.roll(Brin, -1, axis = 1), -1, axis = 3)[p, ir1:ir2, -1, 1:-2] \
    ...
    B2d[p, ir1:ir2, 1:-1, 0] = h22d[p, ir1:ir2, :-1, 0, 5] * B2in[p, ir1:ir2, 1:-1, 0] \
                                   ...
                                   + 0.5 * hr2d[p, ir1:ir2, :-1, 0, 5] * (Brin[p, ir1:ir2, 1:-1, 0] + N.roll(Brin, -1, axis = 1)[p, ir1:ir2, 1:-1, 0])
    # Top edge
    B2d[p, ir1:ir2, 1:-1, -1] = h22d[p, ir1:ir2, :-1, -1, 5] * B2in[p, ir1:ir2, 1:-1, -1] \
                                   ...
                                   + 0.5 * hr2d[p, ir1:ir2, :-1, -1, 5] * (Brin[p, ir1:ir2, 1:-1, -1] + N.roll(Brin, -1, axis = 1)[p, ir1:ir2, 1:-1, -1])
    # Bottom-left corner
    B2d[p, ir1:ir2, 0, 0] = h22d[p, ir1:ir2, 0, 0, 4] * B2in[p, ir1:ir2, 0, 0] \
                                + h12d[p, ir1:ir2, 0, 0, 4] * B1in[p, ir1:ir2, 0, 0] \
                                + 0.5 * hr2d[p, ir1:ir2, 0, 0, 4] * (Brin[p, ir1:ir2, 0, 0] + N.roll(Brin, -1, axis = 1)[p, ir1:ir2, 0, 0])
    # Top-left corner
    B2d[p, ir1:ir2, 0, -1] = h22d[p, ir1:ir2, 0, -1, 4] * B2in[p, ir1:ir2, 0, -1] \
                                 + h12d[p, ir1:ir2, 0, -1, 4] * B1in[p, ir1:ir2, 0, -1] \
                                 + 0.5 * hr2d[p, ir1:ir2, 0, -1, 4] * (Brin[p, ir1:ir2, 0, -1] + N.roll(Brin, -1, axis = 1)[p, ir1:ir2, 0, -1])
    # Bottom-right corner
    B2d[p, ir1:ir2, -1, 0] = h22d[p, ir1:ir2, -1, 0, 4] * B2in[p, ir1:ir2, -1, 0] \
                                 + h12d[p, ir1:ir2, -1, 0, 4] * B1in[p, ir1:ir2, -1, 0] \
                                 + 0.5 * hr2d[p, ir1:ir2, -1, 0, 4] * (Brin[p, ir1:ir2, -1, 0] + N.roll(Brin, -1, axis = 1)[p, ir1:ir2, -1, 0])
    # Top-right corner
    B2d[p, ir1:ir2, -1, -1] = h22d[p, ir1:ir2, -1, -1, 4] * B2in[p, ir1:ir2, -1, -1] \
                                  + h12d[p, ir1:ir2, -1, -1, 4] * B1in[p, ir1:ir2, -1, -1] \
                                  + 0.5 * hr2d[p, ir1:ir2, -1, -1, 4] * (Brin[p, ir1:ir2, -1, -1] + N.roll(Brin, -1, axis = 1)[p, ir1:ir2, -1, -1])
...
def BC_Hd(patch, Hrin, H1in, H2in):
    H1in[patch, (Nr0 + NG), :, :] = H1in[patch, (Nr0 + NG) - 1, :, :]
    H2in[patch, (Nr0 + NG), :, :] = H2in[patch, (Nr0 + NG) - 1, :, :]

def BC_Du(patch, Drin, D1in, D2in):
    Drin[patch, (Nr0 + NG), :, :] = Drin[patch, (Nr0 + NG) - 1, :, :] ## ????
    D1in[patch, NG - 1, :, :] = D1in[patch, NG, :, :]
    D2in[patch, NG - 1, :, :] = D2in[patch, NG, :, :]    

def BC_Bu(patch, Brin, B1in, B2in):
    Brin[patch, NG - 1, :, :] = Brin[patch, NG, :, :] ## ????
    B1in[patch, (Nr0 + NG), :, :] = B1in[patch, (Nr0 + NG) - 1, :, :]
    B2in[patch, (Nr0 + NG), :, :] = B2in[patch, (Nr0 + NG) - 1, :, :]
...
def compute_penalty_D(p0, p1, dtin, Drin, D1in, D2in, Brin, B1in, B2in):
    ...
    if (top == 'xx'):
        ...
        lambda_0 = alpha_int[p0, :, :, loc.right] * N.sqrt(h11d_int[p0, :, :, loc.right])
        lambda_1 = alpha_int[p1, :, :, loc.left]  * N.sqrt(h11d_int[p1, :, :, loc.left]) 
        ...
        Dr_1 = Drin[p1, :, 0, :]
        D1_1, D2_1 = transform_vect(p1, p0, xi_int[None, 0], eta_int[None, :], D1in[p1, :, 0, :], interp(D2in[p1, :, 0, :], eta_half, eta_int))
        B2_1 = B2in[p1, :, 0, :]
        ...
        Dr_0 = Drin[p0, :, -1, :]
        D1_0, D2_0 = transform_vect(p0, p1, xi_int[None, -1], eta_int[None, :], D1in[p0, :, -1, :], interp(D2in[p0, :, -1, :], eta_half, eta_int))
        B2_0 = B2in[p0, :, -1, :]
        ...
        lambda_0 = alpha_half[p0, :, :, loc.right] * N.sqrt(h11d_half[p0, :, :, loc.right])
        lambda_1 = alpha_half[p1, :, :, loc.left]  * N.sqrt(h11d_half[p1, :, :, loc.left]) 

        D1_0 = interp(D1in[p0, :, -1, :], eta_int, eta_half)
        D2_0 = D2in[p0, :, -1, :]
        Br_0 = Brin[p0, :, -1, :]
        
        D1_1, D2_1 = transform_vect(p1, p0, xi_half[None, 0], eta_half[None, :], interp(D1in[p1, :, 0, :], eta_int, eta_half), D2in[p1, :, 0, :])
        Br_1 = Brin[p1, :, 0, :]
        ...
        D1_0, D2_0 = transform_vect(p0, p1, xi_half[None, -1], eta_half[None, :], interp(D1in[p0, :, -1, :], eta_int, eta_half), D2in[p0, :, -1, :])
        Br_0 = Brin[p0, :, -1, :]
    ...
    if (top == 'xy'):
        ...
        lambda_0 = alpha_int[p0, :, :, loc.right]  * N.sqrt(h11d_int[p0, :, :, loc.right]) 
        lambda_1 = alpha_int[p1, :, :, loc.bottom] * N.sqrt(h22d_int[p1, :, :, loc.bottom])
        ...
        Dr_1 = Drin[p1, :, :, 0]
        D1_1, D2_1 = transform_vect(p1, p0, xi_int[None, :], eta_int[None, 0], interp(D1in[p1, :, :, 0], xi_half, xi_int), D2in[p1, :, :, 0])
        B1_1 = B1in[p1, :, :, 0]
        ...
        lambda_0 = alpha_half[p0, :, :, loc.right]  * N.sqrt(h11d_half[p0, :, :, loc.right]) 
        lambda_1 = alpha_half[p1, :, :, loc.bottom] * N.sqrt(h22d_half[p1, :, :, loc.bottom])

        D1_0 = interp(D1in[p0, :, -1, :], eta_int, eta_half)
        D2_0 = D2in[p0, :, -1, :]
        Br_0 = Brin[p0, :, -1, :]

        D1_1, D2_1 = transform_vect(p1, p0, xi_half[None, :], eta_half[None, 0], D1in[p1, :, :, 0], interp(D2in[p1, :, :, 0], xi_int, xi_half))
        Br_1 = Brin[p1, :, :, 0]
    ...
    if (top == 'yy'):
        ...
        lambda_0 = alpha_int[p0, :, :, loc.top]    * N.sqrt(h22d_int[p0, :, :, loc.top])   
        lambda_1 = alpha_int[p1, :, :, loc.bottom] * N.sqrt(h22d_int[p1, :, :, loc.bottom])
        ...
        Dr_0 = Drin[p0, :, :, -1]
        D1_0, D2_0 = transform_vect(p0, p1, xi_int[None, :], eta_int[None, -1], interp(D1in[p0, :, :, -1], xi_half, xi_int), D2in[p0, :, :, -1])
        B1_0 = B1in[p0, :, :, -1]
        ...
        lambda_0 = alpha_half[p0, :, :, loc.top]    * N.sqrt(h22d_half[p0, :, :, loc.top])   
        lambda_1 = alpha_half[p1, :, :, loc.bottom] * N.sqrt(h22d_half[p1, :, :, loc.bottom])
        ...
        D1_0, D2_0 = transform_vect(p0, p1, xi_half[None, :], eta_half[None, -1], D1in[p0, :, :, -1], interp(D2in[p0, :, :, -1], xi_int, xi_half))
        Br_0 = Brin[p0, :, :, -1]
...
def compute_penalty_B(p0, p1, dtin, Drin, D1in, D2in, Brin, B1in, B2in):
    ...
    if (top == 'xx'):
        ...
        D2_1 = D2in[p1, :,  0, :]
        Br_1 = Brin[p1, :,  0, :]
        B1_1, B2_1 = transform_vect(p1, p0, xi_half[None, 0], eta_half[None, :], B1in[p1, :,  0, :], interp(B2in[p1, :,  0, :], eta_int, eta_half))
        ...
        D2_0 = D2in[p0, :,  -1, :]
        Br_0 = Brin[p0, :,  -1, :]
        B1_0, B2_0 = transform_vect(p0, p1, xi_half[None, -1], eta_half[None, :], B1in[p0, :,  -1, :], interp(B2in[p0, :,  -1, :], eta_int, eta_half))
        ...
        Dr_1 = Drin[p1, :,  0, :]
        B1_1, B2_1 = transform_vect(p1, p0, xi_int[None, 0], eta_int[None, :], interp(B1in[p1, :,  0, :], eta_half, eta_int), B2in[p1, :,  0, :])
        ...
        Dr_0 = Drin[p0, :,  -1, :]
        B1_0, B2_0 = transform_vect(p0, p1, xi_int[None, -1], eta_int[None, :], interp(B1in[p0, :,  -1, :], eta_half, eta_int), B2in[p0, :,  -1, :])
    ...
    if (top == 'xy'):
        ...
        D1_1 = D1in[p1, :,  :, 0]
        Br_1 = Brin[p1, :,  :, 0]
        B1_1, B2_1 = transform_vect(p1, p0, xi_half[None, :], eta_half[None, 0], interp(B1in[p1, :,  :, 0], xi_int, xi_half), B2in[p1, :,  :, 0])
        ...
        Dr_1 = Drin[p1, :,  :, 0]
        B1_1, B2_1 = transform_vect(p1, p0, xi_int[None, :], eta_int[None, 0], B1in[p1, :,  :, 0], interp(B2in[p1, :,  :, 0], xi_half, xi_int))
    ...
    if (top == 'yy'):
        ...
        D1_0 = D1in[p0, :,  :, -1]
        Br_0 = Brin[p0, :,  :, -1]
        B1_0, B2_0 = transform_vect(p0, p1, xi_half[None, :], eta_half[None, -1], interp(B1in[p0, :,  :, -1], xi_int, xi_half), B2in[p0, :,  :, -1])
        ...
        Dr_0 = Drin[p0, :,  :, -1]
        B1_0, B2_0 = transform_vect(p0, p1, xi_int[None, :], eta_int[None, -1], B1in[p0, :,  :, -1], interp(B2in[p0, :,  :, -1], xi_half, xi_int))
...
def interface_D(p0, p1, Drin, D1in, D2in):
    ...
    if (top == 'xx'):
        ...
        D2in[p0, ir1:ir2, -1, i0:i1_half] -= diff_D2u[p0, ir1:ir2, -1, i0:i1_half] / sqrt_det_h_half[p0, ir1:ir2, i0:i1_half, loc.right]
        D2in[p1, ir1:ir2, 0, i0:i1_half]  -= diff_D2u[p1, ir1:ir2, 0, i0:i1_half]  / sqrt_det_h_half[p1, ir1:ir2, i0:i1_half, loc.left]
    ...
    if (top == 'xy'):
        ...
        D2in[p0, ir1:ir2, -1, i0:i1_half] -= diff_D2u[p0, ir1:ir2, -1, i0:i1_half] / sqrt_det_h_half[p0, ir1:ir2, i0:i1_half, loc.right]
        D1in[p1, ir1:ir2, i0:i1_half, 0]  -= diff_D1u[p1, ir1:ir2, i0:i1_half, 0]  / sqrt_det_h_half[p1, ir1:ir2, i0:i1_half, loc.bottom]

    if (top == 'yy'):
        Drin[p0, ir1:ir2, i0:i1_int, -1] -= diff_Dru[p0, ir1:ir2, i0:i1_int, -1] / sqrt_det_h_int[p0, ir1:ir2, i0:i1_int, loc.top]
        Drin[p1, ir1:ir2, i0:i1_int, 0]  -= diff_Dru[p1, ir1:ir2, i0:i1_int, 0]  / sqrt_det_h_int[p1, ir1:ir2, i0:i1_int, loc.bottom]

        D1in[p0, ir1:ir2, i0:i1_half, -1] -= diff_D1u[p0, ir1:ir2, i0:i1_half, -1] / sqrt_det_h_half[p0, ir1:ir2, i0:i1_half, loc.top]
        D1in[p1, ir1:ir2, i0:i1_half, 0]  -= diff_D1u[p1, ir1:ir2, i0:i1_half, 0]  / sqrt_det_h_half[p1, ir1:ir2, i0:i1_half, loc.bottom]
...
def corners_D(p0, Drin, D1in, D2in):
    ...
    Drin[p0, ir1:ir2, 0, 0]   -= diff_Dru[p0, ir1:ir2, 0, 0]   * sig_in / sqrt_det_h_int[p0, ir1:ir2, 0, loc.bottom]
    Drin[p0, ir1:ir2, -1, 0]  -= diff_Dru[p0, ir1:ir2, -1, 0]  * sig_in / sqrt_det_h_int[p0, ir1:ir2, -1, loc.bottom]
    Drin[p0, ir1:ir2, 0, -1]  -= diff_Dru[p0, ir1:ir2, 0, -1]  * sig_in / sqrt_det_h_int[p0, ir1:ir2, 0, loc.top]
    Drin[p0, ir1:ir2, -1, -1] -= diff_Dru[p0, ir1:ir2, -1, -1] * sig_in / sqrt_det_h_int[p0, ir1:ir2, -1, loc.top]

    D1in[p0, ir1:ir2, 0, 0]   -= diff_D1u[p0, ir1:ir2, 0, 0]   * sig_in / sqrt_det_h_half[p0, ir1:ir2, 0, loc.bottom]
    D1in[p0, ir1:ir2, -1, 0]  -= diff_D1u[p0, ir1:ir2, -1, 0]  * sig_in / sqrt_det_h_half[p0, ir1:ir2, -1, loc.bottom]
    D1in[p0, ir1:ir2, 0, -1]  -= diff_D1u[p0, ir1:ir2, 0, -1]  * sig_in / sqrt_det_h_half[p0, ir1:ir2, 0, loc.top] 
    D1in[p0, ir1:ir2, -1, -1] -= diff_D1u[p0, ir1:ir2, -1, -1] * sig_in / sqrt_det_h_half[p0, ir1:ir2, -1, loc.top]

    D2in[p0, ir1:ir2, 0, 0]   -= diff_D2u[p0, ir1:ir2, 0, 0]   * sig_in / sqrt_det_h_half[p0, ir1:ir2, 0, loc.bottom]
    D2in[p0, ir1:ir2, -1, 0]  -= diff_D2u[p0, ir1:ir2, -1, 0]  * sig_in / sqrt_det_h_half[p0, ir1:ir2, -1, loc.bottom]
    D2in[p0, ir1:ir2, 0, -1]  -= diff_D2u[p0, ir1:ir2, 0, -1]  * sig_in / sqrt_det_h_half[p0, ir1:ir2, 0, loc.top]
    D2in[p0, ir1:ir2, -1, -1] -= diff_D2u[p0, ir1:ir2, -1, -1] * sig_in / sqrt_det_h_half[p0, ir1:ir2, -1, loc.top]
...
def interface_B(p0, p1, Brin, B1in, B2in):
    ...
    if (top == 'xx'):
        Brin[p0, ir1:ir2, -1, i0:i1_half] -= diff_Bru[p0, ir1:ir2, -1, i0:i1_half] / sqrt_det_h_half[p0, ir1:ir2, i0:i1_half, loc.right]
        Brin[p1, ir1:ir2, 0, i0:i1_half]  -= diff_Bru[p1, ir1:ir2, 0, i0:i1_half]  / sqrt_det_h_half[p1, ir1:ir2, i0:i1_half, loc.left]
    ...
    if (top == 'xy'):
        Brin[p0, ir1:ir2, -1, i0:i1_half] -= diff_Bru[p0, ir1:ir2, -1, i0:i1_half] / sqrt_det_h_half[p0, ir1:ir2, i0:i1_half, loc.right]
        Brin[p1, ir1:ir2, i0:i1_half, 0]  -= diff_Bru[p1, ir1:ir2, i0:i1_half, 0]  / sqrt_det_h_half[p1, ir1:ir2, i0:i1_half, loc.bottom]

        B2in[p0, ir1:ir2, -1, i0:i1_int] -= diff_B2u[p0, ir1:ir2, -1, i0:i1_int] / sqrt_det_h_int[p0, ir1:ir2, i0:i1_int, loc.right]
        B1in[p1, ir1:ir2, i0:i1_int, 0]  -= diff_B1u[p1, ir1:ir2, i0:i1_int, 0]  / sqrt_det_h_int[p1, ir1:ir2, i0:i1_int, loc.bottom]

    if (top == 'yy'):
        Brin[p0, ir1:ir2, i0:i1_half, -1] -= diff_Bru[p0, ir1:ir2, i0:i1_half, -1] / sqrt_det_h_half[p0, ir1:ir2, i0:i1_half, loc.top]
        Brin[p1, ir1:ir2, i0:i1_half, 0]  -= diff_Bru[p1, ir1:ir2, i0:i1_half, 0]  / sqrt_det_h_half[p1, ir1:ir2, i0:i1_half, loc.bottom]
...
def corners_B(p0, Brin, B1in, B2in):
    ...
    Brin[p0, ir1:ir2, 0, 0]   -= diff_Bru[p0, ir1:ir2, 0, 0]   * sig_in / sqrt_det_h_half[p0, ir1:ir2, 0, loc.bottom]
    Brin[p0, ir1:ir2, -1, 0]  -= diff_Bru[p0, ir1:ir2, -1, 0]  * sig_in / sqrt_det_h_half[p0, ir1:ir2, -1, loc.bottom] 
    Brin[p0, ir1:ir2, 0, -1]  -= diff_Bru[p0, ir1:ir2, 0, -1]  * sig_in / sqrt_det_h_half[p0, ir1:ir2, 0, loc.top]
    Brin[p0, ir1:ir2, -1, -1] -= diff_Bru[p0, ir1:ir2, -1, -1] * sig_in / sqrt_det_h_half[p0, ir1:ir2, -1, loc.top] 

    B1in[p0, ir1:ir2, 0, 0]   -= diff_B1u[p0, ir1:ir2, 0, 0] * sig_in   / sqrt_det_h_int[p0, ir1:ir2, 0, loc.bottom]
    B1in[p0, ir1:ir2, -1, 0]  -= diff_B1u[p0, ir1:ir2, -1, 0] * sig_in  / sqrt_det_h_int[p0, ir1:ir2, -1, loc.bottom] 
    B1in[p0, ir1:ir2, 0, -1]  -= diff_B1u[p0, ir1:ir2, 0, -1] * sig_in  / sqrt_det_h_int[p0, ir1:ir2, 0, loc.top]
    B1in[p0, ir1:ir2, -1, -1] -= diff_B1u[p0, ir1:ir2, -1, -1] * sig_in / sqrt_det_h_int[p0, ir1:ir2, -1, loc.top] 

    B2in[p0, ir1:ir2, 0, 0]   -= diff_B2u[p0, ir1:ir2, 0, 0] * sig_in   / sqrt_det_h_int[p0, ir1:ir2, 0, loc.bottom]
    B2in[p0, ir1:ir2, -1, 0]  -= diff_B2u[p0, ir1:ir2, -1, 0] * sig_in  / sqrt_det_h_int[p0, ir1:ir2, -1, loc.bottom] 
    B2in[p0, ir1:ir2, 0, -1]  -= diff_B2u[p0, ir1:ir2, 0, -1] * sig_in  / sqrt_det_h_int[p0, ir1:ir2, 0, loc.top]
    B2in[p0, ir1:ir2, -1, -1] -= diff_B2u[p0, ir1:ir2, -1, -1] * sig_in / sqrt_det_h_int[p0, ir1:ir2, -1, loc.top] 
...
# sqrt_det_hBr = N.zeros((n_patches, Nr, Nxi_half, Neta_half))

# sqrt_det_hBr[:, :, 1:-1, 1:-1] = sqrt_det_h[:, :, :-1, :-1, 3]
# sqrt_det_hBr[:, :, 0, 1:-1]    = sqrt_det_h[:, :, 0, :-1, 2]
# sqrt_det_hBr[:, :, -1, 1:-1]   = sqrt_det_h[:, :, -1, :-1, 2]
# sqrt_det_hBr[:, :, 1:-1, 0]    = sqrt_det_h[:, :, :-1, 0, 1]
# sqrt_det_hBr[:, :, 1:-1, -1]   = sqrt_det_h[:, :, :-1, -1, 1]
# sqrt_det_hBr[:, :, 0, 0]       = sqrt_det_h[:, :, 0, 0, 0]
# sqrt_det_hBr[:, :, -1, 0]      = sqrt_det_h[:, :, -1, 0, 0]
# sqrt_det_hBr[:, :, 0, -1]      = sqrt_det_h[:, :, 0, -1, 0]
# sqrt_det_hBr[:, :, -1, -1]     = sqrt_det_h[:, :, -1, -1, 0]
